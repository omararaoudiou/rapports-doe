  ...
  return (
    React.createElement('div', {}
      ...
      , React.createElement('div', { style: {display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}
        ...
        , !isAntenne && React.createElement('button', { onClick: ()=>setMode(mode==="pico"?"view":"pico"),
          style: {flex:1,padding:"10px 8px",borderRadius:10,border:"none",cursor:"pointer",fontWeight:700,fontSize:13,
...
function PMTable({ plan, pmData, onChange }) {
  ...
  const fillSignalRow = (label) => {
    extractSignalAsync(function(sig) {
      ...
      if (sig.type) filled.operateur = sig.type; // '4G (LTE)' ou '5G (NR)'
      if (filled.g4||filled.g5||filled.speedtest||filled.operateur) {
  ...
  return (
    React.createElement('div', {}
      , pmMarkers.map((m,i) => {
        ...
        return (
          React.createElement('div', { key: m.id, style: (function(){ var c=rsrpBg(d.rsrp||d.g4); return {background:c?c.bg:'#0a1628',borderRadius:12,padding:12,marginBottom:10,border:'1px solid '+(c?c.border:'#1e3a5f')}; })()}
            ...
            , React.createElement('div', { style: {display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:8}}
              , [["g4","4G (RSRP/Band)"],["g5","5G (RSRP/Band)"],["speedtest","Speedtest ↓/↑"],["operateur","Opérateur"]].map(([k,lbl])=>(
