...
// ── getDeviceType: disponible partout dans l'app ──────────────────────────
...
function extractSignalAsync(cb) {
  ...
  if (bridge) {
    var scanFn = null;
    try { if (bridge.startFullScan !== undefined) { bridge.startFullScan(); scanFn = true; } } catch(e1) {}
    if (!scanFn) { try { if (bridge.getLiveSignal !== undefined) { bridge.getLiveSignal(); scanFn = true; } } catch(e2) {} }
    if (!scanFn) { try { if (bridge.startScan !== undefined) { bridge.startScan(); scanFn = true; } } catch(e3) {} }
    if (scanFn) {
      var done = false;
      var handler = function(e) {
        ...
        clearTimeout(timer);
        var info = e.detail || {};
        cb({ type:info.type||'', band:info.band||'', enb:info.enb||info.enbOrGnb||'',
          lcid:info.lcid?String(info.lcid):'', rsrp:info.rsrp?String(info.rsrp):'',
          rsrq:info.rsrq?String(info.rsrq):'',
          snr:info.snr?String(info.snr):(info.rssnr?String(info.rssnr):''),
          dl:info.speed_down||(info.downlinkMbps?String(Math.round(info.downlinkMbps*10)/10):(info.dl?String(info.dl):'')),
          ul:info.speed_up||(info.uplinkMbps?String(Math.round(info.uplinkMbps*10)/10):(info.ul?String(info.ul):'')),
          ping:info.pingMs?String(info.pingMs):(info.ping?String(info.ping):'') });
      };
      window.addEventListener('pico_signal_auto', handler);
      window.addEventListener('android_signal_received', handler);
      var timer = setTimeout(function() {
        if (!done) { done=true; window.removeEventListener('pico_signal_auto',handler); window.removeEventListener('android_signal_received',handler); cb(extractPhysicalSignal()||{}); }
      }, 5000);
      return;
    }
  }
...
function parseNetworkCellInfoText(txt) {
  ...
  var r={band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''};
  var t=txt;
...
async function getApiKey() {
  ...
  try { var r = await window.storage.get('app_config'); if (r) { var cf = JSON.parse(r.value||'{}'); _API_KEY = cf.apiKey||null; } } catch(e) {}
  return _API_KEY;
}
...
function __def(id, fn) {
  var module = { exports: {} };
  var exports = module.exports;
  fn(module, exports, __require);
...
__def("scheduler", function(module, exports, require) {
/**
 * @license React
...
__def("react", function(module, exports, require) {
/**
 * @license React
 * react.production.js
 *
...
// react-dom base (sets __DOM_INTERNALS...)
__def("react-dom", function(module, exports, require) {
/**
 * @license React
...
// react-dom/client (reads react-dom's __DOM_INTERNALS...)
__def("react-dom/client", function(module, exports, require) {
/**
 * @license React
            ...
            default:
              switch (type) {
                ...
                case "script":
                  ...
                  nextResource.innerHTML = "\x3c/script>";
...
var ReactDOM = __require("react-dom/client");
...
async function loadAuditList() { try { const r = await window.storage.get(STORAGE_INDEX_KEY); return r ? JSON.parse(r.value) : []; } catch (e2) { return []; } }
async function saveAuditList(l) { try { await window.storage.set(STORAGE_INDEX_KEY, JSON.stringify(l)); } catch (e3) {} }
async function loadAudit(id) { try { const r = await window.storage.get(`audit2:${id}`); return r ? JSON.parse(r.value) : null; } catch (e4) { return null; } }
async function saveAudit(id, payload) { try { await window.storage.set(`audit2:${id}`, JSON.stringify(payload)); } catch (e5) {} }
async function deleteAuditStorage(id) { try { await window.storage.delete(`audit2:${id}`); } catch (e6) {} }
...
const defaultData = () => ({
  ...
  outils: { notes:"" },
  // plan: image + markers
  plan: { photo:null, markers:[] }, // marker: { id, type:"pm"|"pico", label:"PM01"|"PICO1", x:0.5, y:0.5 }
...
function PlanEditor({ plan, pmData, picoData, onChange, onPmDataChange, onPicoDataChange }) {
  ...
  const handleTap = (e) => {
    ...
    const t = e.touches ? e.touches[0] : e;
    const xPct = (t.clientX - rect.left) / rect.width;
    const yPct = (t.clientY - rect.top) / rect.height;
    ...
    if (mode==="pm") {
      ...
      const newPicoData = { ...picoData, [label]: { photo:null, hauteur:"", cablage:"", notes:"" } };
      onChange({ ...plan, markers:[...plan.markers, newMarker] });
  ...
  return (
    React.createElement('div', {}
      ...
      , React.createElement('div', { style: {display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}
        , React.createElement('button', { onClick: ()=>setMode(mode==="pm"?"view":"pm"),
        ...
        , React.createElement('button', { onClick: ()=>setMode(mode==="pico"?"view":"pico"),
          style: {flex:1,padding:"10px 8px",borderRadius:10,border:"none",cursor:"pointer",fontWeight:700,fontSize:13,
      ...
      , React.createElement('div', { ref: containerRef, style: {position:"relative",borderRadius:12,overflow:"hidden",border:"1px solid #1e3a5f",cursor:mode==="view"?"default":"crosshair"},
        ...
        , React.createElement('img', { ref: imgRef, src: plan.photo, alt: "Plan du site"  , style: {width:"100%",display:"block"}} )
...
function PMTable({ plan, pmData, onChange }) {
  ...
  if (pmMarkers.length===0) return (
    React.createElement('div', { style: {background:"#0a1628",borderRadius:10,padding:16,textAlign:"center",color:"#334155",fontSize:13}}, "Ajoutez des points de mesure sur le plan pour les voir apparaître ici."

    )
  );
  const upd = (label,k,v) => onChange({...pmData, [label]:{...pmData[label],[k]:v}});

  const fillSignalRow = (label) => {
    extractSignalAsync(function(sig) {
      if (!sig || !Object.values(sig).some(v=>v!=='')) return;
      var filled = {};
      var g4parts = [sig.type&&sig.type, sig.band&&('Band:'+sig.band), sig.rsrp&&('RSRP:'+sig.rsrp+'dBm'), sig.rsrq&&('RSRQ:'+sig.rsrq+'dBm')].filter(Boolean);
      if (g4parts.length>0) filled.g4 = g4parts.join(' / ');
      if (sig.snr) filled.g5 = 'SNR:'+sig.snr+(parseInt(sig.snr)>10?' ✓':'');
      var spparts = [sig.dl&&('↓'+sig.dl+'Mbps'), sig.ul&&('↑'+sig.ul+'Mbps'), sig.ping&&('ping:'+sig.ping+'ms')].filter(Boolean);
      if (spparts.length>0) filled.speedtest = spparts.join(' / ');
      if (filled.g4||filled.g5||filled.speedtest) {
        onChange({...pmData,[label]:{...(pmData[label]||{}),...filled}});
      }
    });
  };
  ...
  return (
    React.createElement('div', {}
      , pmMarkers.map((m,i) => {
        const d = pmData[m.label] || { g4:"", g5:"", speedtest:"", notes:"" };
        return (
          React.createElement('div', { key: m.id, style: {background:"#0a1628",borderRadius:12,padding:12,marginBottom:10,border:"1px solid #1e3a5f"}}
            , React.createElement('div', { style: {display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:8}}
              , React.createElement('div', {style:{display:'flex',alignItems:'center',gap:8}}
                , React.createElement('div', { style: {background:"#3b82f6",borderRadius:12,padding:"3px 10px",fontSize:12,fontWeight:800,color:"#fff"}}, m.label)
                , React.createElement('span', { style: {fontSize:12,color:"#475569"}}, "Point " , i+1)
              )
              , React.createElement('button', {
                  onClick:()=>fillSignalRow(m.label),
                  style:{background:'linear-gradient(135deg,#22c55e,#16a34a)',border:'none',borderRadius:8,color:'#fff',padding:'6px 12px',cursor:'pointer',fontSize:11,fontWeight:700,display:'flex',alignItems:'center',gap:5,flexShrink:0}
                }, '📶 Pré-remplir')
            )
            , React.createElement('div', { style: {display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:8}}
              , [["g4","4G (RSRP/Band)"],["g5","5G (RSRP/Band)"],["speedtest","Speedtest ↓/↑"]].map(([k,lbl])=>(
                React.createElement('div', { key: k}
                  , React.createElement('div', { style: {fontSize:10,color:"#475569",marginBottom:3,fontWeight:700}}, lbl)
                  , React.createElement('input', { value: d[k]||"", onChange: e=>upd(m.label,k,e.target.value), placeholder: "—",
                    style: {width:"100%",padding:"7px 8px",borderRadius:7,border:"1px solid #1e3a5f",background:"#0f2040",color:"#e2e8f0",fontSize:12,boxSizing:"border-box"}} )
                )
              ))
            )
            , React.createElement('input', { value: d.notes||"", onChange: e=>upd(m.label,"notes",e.target.value), placeholder: "Notes (localisation, observations...)"  ,
              style: {width:"100%",padding:"8px 10px",borderRadius:8,border:"1px solid #1e3a5f",background:"#0f2040",color:"#e2e8f0",fontSize:12,boxSizing:"border-box"}} )
          )
        );
      })
    )
        ...
        , photos.map((p,i) => (
          React.createElement('div', { key: i, style: {background:"#0a1628",borderRadius:12,overflow:"hidden",border:"1px solid #1e3a5f"}}
            , React.createElement('div', { style: {position:"relative"}}
              , React.createElement('img', { src: p.annotated||p.src, alt: "", style: {width:"100%",height:120,objectFit:"cover",display:"block"}} )
...
function PDFPreview({ data, onClose, auditType }) {
  const pmMarkers  = data.plan.markers.filter(m=>m.type==="pm");
  const picoMarkers= data.plan.markers.filter(m=>m.type==="pico");
  return (
    React.createElement('div', { style: {position:"fixed",inset:0,background:"#000c",zIndex:3000,overflowY:"auto",padding:"20px 12px"}}
      , React.createElement('div', { style: {maxWidth:800,margin:"0 auto",background:"#fff",borderRadius:16,overflow:"hidden"}}
        ...
        , React.createElement('div', { style: {padding:24,color:"#1e293b"}}
          ...
          /* Photos */
          , [...data.local.photos,...data.reportingPhotos.photos].length>0 && (
            React.createElement('div', { style: {marginBottom:20}}
              , React.createElement('h3', { style: {fontSize:15,fontWeight:800,color:"#0f2040",marginBottom:10}}, "Reporting photos" )
              , React.createElement('div', { style: {display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}
                , [...data.local.photos.map(p=>({...p,s:"Local"})),...data.reportingPhotos.photos.map(p=>({...p,s:"Reporting"}))].map((p,i)=>(
                  React.createElement('div', { key: i}, React.createElement('img', { src: p.annotated||p.src, alt: "", style: {width:"100%",borderRadius:8,objectFit:"cover",height:120}}), React.createElement('div', { style: {fontSize:10,color:"#64748b",marginTop:4}}, String(i+1).padStart(2,"0"), " — "  , p.s))
                ))
              )
            )
          )
          ...
          , (data.oeuvre.nbTechniciens || data.oeuvre.nacelle !== undefined) && (
            React.createElement('div', { style: {marginBottom:20}}
              ...
              /* Cable total */
              , data.plan.markers.filter(m=>m.type==="pico").length>0 && (()=>{
                const pics = data.plan.markers.filter(m=>m.type==="pico");
              ...
              , React.createElement('div', { style: {background:"#f8fafc",borderRadius:8,padding:14,fontSize:13,lineHeight:1.9}}
                , data.plan.markers.filter(m=>m.type==="pico").map(m=>{const pd=data.picoData[m.label]||{};return pd.cablage?React.createElement('div', { key: m.label}, React.createElement('strong', {}, m.label, " :" ), " " , pd.cablage, "m câble "  , pd.hauteur?`+ ${pd.hauteur}m hauteur`:""):null;})
                ...
                , Object.entries(data.oeuvre.fournitures||{}).filter(([,v])=>v).map(([k])=>{
                  const nb=data.plan.markers.filter(m=>m.type==="pico").length;
...
function defaultAntenneData() {
  return {
    ...
    antenne: {
      ...
      mesures: [
        {label:'Test 1 - emplacement antenne externe', band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''},
        {label:'Test 2 - emplacement antenne externe', band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''},
        {label:'Test 3 - emplacement antenne externe', band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''},
        {label:'Test 1 - emplacement routeur',         band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''},
        {label:'Test 2 - emplacement routeur',         band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''},
        {label:'Test 3 - emplacement routeur',         band:'',enb:'',lcid:'',rsrp:'',rsrq:'',snr:'',ul:'',dl:'',ping:''},
      ],
...
var DOC_TYPES = [
  { id:'audit',   label:'Audit PICO BTS',          icon:'📡', color:'#3b82f6', border:'#1e3a5f',  bg:'linear-gradient(135deg,#0f2040,#0a1628)', desc:'Audit de couverture mobile 4G/5G' },
  { id:'doe',     label:'DOE PICO BTS',            icon:'📋', color:'#22c55e', border:'#22c55e40', bg:'linear-gradient(135deg,#052010,#0a1628)', desc:'Document des ouvrages exécutés' },
  { id:'antenne', label:'Audit Antenne Déportée',  icon:'📶', color:'#f97316', border:'#7c2d12',   bg:'linear-gradient(135deg,#1f0a00,#0a1628)', desc:'Audit déploiement antenne 4G/5G déportée' },
];
...
function DocTypeSelector({ onCreate }) {
  ...
  return React.createElement('div', { style: {marginBottom:28} },
    React.createElement('div', { style: {display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12} },
      ...
      }, React.createElement('span',{style:{fontSize:16}},'+'), ' Créer')
    ),
    showMenu && React.createElement('div', { style: {background:'#0f2040',borderRadius:16,border:'1px solid #1e3a5f',overflow:'hidden',boxShadow:'0 8px 32px #000a'} },
      DOC_TYPES.map(function(dt) {
        return React.createElement('button', {
          key: dt.id,
          onClick: function(){ setShowMenu(false); onCreate(dt.id); },
          style: {width:'100%',padding:'16px 20px',border:'none',borderBottom:'1px solid #1e3a5f',background:dt.bg,cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:16}
        },
          React.createElement('div', { style: {width:44,height:44,borderRadius:12,background:dt.color+'20',border:'1px solid '+dt.border,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0} }, dt.icon),
          React.createElement('div', { style: {flex:1,minWidth:0} },
            React.createElement('div', { style: {fontWeight:700,fontSize:14,color:dt.color} }, dt.label),
            React.createElement('div', { style: {fontSize:11,color:'#475569',marginTop:3} }, dt.desc)
          ),
          React.createElement('span', { style: {color:'#334155',fontSize:18} }, '›')
        );
      })
    )
  );
...
function ProfileScreen(props) {
  var profile = props.profile, onSave = props.onSave, onClose = props.onClose;
  ...
  function handleSave() {
    if (showPw) {
      ...
      if (pwNew !== pwConf)  { setMsg('❌ Mots de passe ne correspondent pas'); return; }
    }
    var updated = Object.assign({}, profile, {
      name:name, phone:phone, defaultEmail:email, fonction:fct, direction:dir,
    });
    if (showPw && pwNew) updated.pwdHash = hashPwd(pwNew);
    var profiles = getProfiles();
    saveProfiles(profiles.map(function(p){ return p.id===profile.id ? updated : p; }));
    setMsg('✅ Profil sauvegardé');
    if (typeof window._activeProfile !== 'undefined') window._activeProfile = profile.id;
    setTimeout(function(){ onSave(updated); onClose(); }, 900);
  }
  ...
  function disconnectDevice(type) {
    ...
    var profiles = getProfiles();
    saveProfiles(profiles.map(function(p){ return p.id===profile.id ? updated : p; }));
    setMsg('✅ Appareil déconnecté'); setTimeout(function(){ onSave(updated); onClose(); }, 700);
  }

  var F  = {width:'100%',padding:'11px 14px',borderRadius:10,border:'1px solid #1e3a5f',
  ...
  var Cd = {background:'#0f2040',borderRadius:12,padding:'16px',marginBottom:12,border:'1px solid #1e3a5f'};
  var CdT= {fontSize:12,fontWeight:700,color:'#93c5fd',textTransform:'uppercase',
    letterSpacing:'0.06em',marginBottom:12,paddingBottom:8,borderBottom:'1px solid #1e3a5f'};
  var e  = React.createElement;

  return e('div',{style:{position:'fixed',inset:0,background:'#020817',zIndex:9999,overflowY:'auto',fontFamily:'inherit',display:'flex',flexDirection:'column'}},
    e('div',{style:{background:'#0f2040',padding:'14px 16px',display:'flex',alignItems:'center',gap:12,borderBottom:'1px solid #1e3a5f',flexShrink:0}},
      e('button',{onClick:onClose,style:{background:'transparent',border:'none',color:'#64748b',cursor:'pointer',fontSize:20,padding:'0 4px'}},'←'),
      e('div',{style:{flex:1}},
        e('div',{style:{fontSize:16,fontWeight:700,color:'#e2e8f0'}},'Mon Profil'),
        e('div',{style:{fontSize:11,color:'#475569'}},profile.username)
      ),
      e('button',{onClick:handleSave,style:{padding:'8px 16px',borderRadius:8,border:'none',background:'#22c55e',color:'#fff',cursor:'pointer',fontWeight:700,fontSize:13}},'💾 Sauvegarder')
    ),
    e('div',{style:{padding:'16px',maxWidth:600,margin:'0 auto',width:'100%'}},
      e('div',{style:Cd},
        e('div',{style:CdT},'🔒 Identifiant (non modifiable)'),
        e('label',{style:L},'Identifiant'), e('input',{value:profile.username||'',readOnly:true,style:Fg}),
        e('label',{style:L},'Rôle'), e('input',{value:profile.isAdmin?'Administrateur':profile.isResponsable?'Responsable':'Technicien',readOnly:true,style:Fg})
      ),
      e('div',{style:Cd},
        e('div',{style:CdT},'👤 Informations personnelles'),
        e('label',{style:L},'Nom complet'), e('input',{value:name,onChange:function(ev){setName(ev.target.value);},placeholder:'ex: Emmanuel MAUDET',style:F}),
        e('label',{style:L},'Téléphone'), e('input',{value:phone,onChange:function(ev){setPhone(ev.target.value);},placeholder:'06 12 34 56 78',style:F,type:'tel'}),
        e('label',{style:L},'Email par défaut (rapports)'), e('input',{value:email,onChange:function(ev){setEmail(ev.target.value);},placeholder:'technicien@sogetrel.fr',style:F,type:'email'}),
        e('label',{style:L},'Fonction'), e('input',{value:fct,onChange:function(ev){setFct(ev.target.value);},placeholder:'Technicien Réseaux & Télécoms.',style:F}),
        e('label',{style:L},'Direction'), e('input',{value:dir,onChange:function(ev){setDir(ev.target.value);},placeholder:'Direction des Opérations Ile De France',style:F})
      ),
      e('div',{style:Cd},
        e('div',{style:CdT},'📱 Appareils associés (1 mobile + 1 ordinateur max)'),
        [['mobile','📱','Mobile',mobileAssoc,isCurrentMobile],['desktop','🖥️','Ordinateur',desktopAssoc,isCurrentDesktop]].map(function(row){
          var type=row[0],icon=row[1],lbl=row[2],assoc=row[3],isCur=row[4];
          return e('div',{key:type,style:{padding:'10px 12px',borderRadius:8,marginBottom:8,background:assoc?'#052e16':'#0a1628',border:'1px solid '+(assoc?'#22c55e40':'#1e3a5f'),display:'flex',alignItems:'center',gap:10}},
            e('span',{style:{fontSize:18}},icon),
            e('div',{style:{flex:1}},
              e('div',{style:{fontSize:13,fontWeight:700,color:assoc?'#22c55e':'#475569'}},assoc?(isCur?'✓ Cet appareil (actuel)':lbl+' associé'):'Aucun '+lbl.toLowerCase()+' associé'),
              assoc&&!isCur&&e('div',{style:{fontSize:11,color:'#475569'}},assoc.substring(0,16)+'...')
            ),
            assoc&&!isCur&&e('button',{onClick:function(){disconnectDevice(type);},style:{padding:'5px 10px',borderRadius:6,border:'none',background:'#450a0a',color:'#ef4444',cursor:'pointer',fontSize:11}},'Déconnecter')
          );
        }),
        e('div',{style:{fontSize:11,color:'#475569',marginTop:6}},'📌 1 mobile + 1 ordinateur par compte. Déconnecter pour changer.')
      ),
      e('div',{style:Cd},
        e('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:showPw?12:0}},
          e('div',{style:CdT},'🔑 Mot de passe'),
          e('button',{onClick:function(){setShowPw(!showPw);setMsg('');},style:{padding:'6px 12px',borderRadius:8,border:'1px solid #1e3a5f',background:showPw?'#1e3a5f':'transparent',color:'#93c5fd',cursor:'pointer',fontSize:12}},showPw?'Annuler':'Changer')
        ),
        !showPw&&e('div',{style:{fontSize:12,color:'#475569'}},profile.pwdHash?'••••••••':'Aucun mot de passe'),
        showPw&&e('div',null,
          e('label',{style:L},'Mot de passe actuel'), e('input',{value:pwOld,onChange:function(ev){setPwOld(ev.target.value);},type:'password',placeholder:'Mot de passe actuel',style:F}),
          e('label',{style:L},'Nouveau mot de passe'), e('input',{value:pwNew,onChange:function(ev){setPwNew(ev.target.value);},type:'password',placeholder:'Minimum 4 caractères',style:F}),
          e('label',{style:L},'Confirmer'), e('input',{value:pwConf,onChange:function(ev){setPwConf(ev.target.value);},type:'password',placeholder:'Confirmer',style:F})
        )
      ),
      msg&&e('div',{style:{padding:'12px 16px',borderRadius:10,textAlign:'center',background:msg.startsWith('✅')?'#052e16':'#450a0a',color:msg.startsWith('✅')?'#22c55e':'#ef4444',fontSize:14,fontWeight:600,marginBottom:12}},msg)
    )
  );
}
...
function ResponsableScreen(props) {
  var profile = props.profile, onClose = props.onClose;
  ...
  React.useEffect(function(){
    ...
    loadAuditList().then(function(list){
      var techAudits = (list||[]).filter(function(a){
        var g=a.garde||{};
        return g.technicien&&g.technicien.toLowerCase().includes(selTech.name.toLowerCase());
      });
      setAudits(techAudits); setLoading(false);
    });
  }, [selTech]);

  var COLORS=['#3b82f6','#22c55e','#f97316','#a855f7','#ef4444','#06b6d4'];

  return e('div',{style:{position:'fixed',inset:0,background:'#020817',zIndex:9999,display:'flex',flexDirection:'column',fontFamily:'inherit'}},
    e('div',{style:{background:'linear-gradient(135deg,#0c2040,#1a3a60)',padding:'14px 16px',display:'flex',alignItems:'center',gap:12,borderBottom:'1px solid #1e3a5f',flexShrink:0}},
      e('button',{onClick:onClose,style:{background:'transparent',border:'none',color:'#64748b',cursor:'pointer',fontSize:20}},'←'),
      e('div',{style:{flex:1}},
        e('div',{style:{fontSize:15,fontWeight:700,color:'#e2e8f0'}},'👥 Mes Techniciens'),
        e('div',{style:{fontSize:11,color:'#64748b'}},myTechs.length+' technicien'+(myTechs.length>1?'s':''))
      )
    ),
    e('div',{style:{flex:1,overflowY:'auto',padding:16}},
      myTechs.length===0&&e('div',{style:{textAlign:'center',padding:'32px 0',color:'#334155'}},
        e('div',{style:{fontSize:32,marginBottom:8}},'👤'),
        "Aucun technicien affecté. Demandez à l'administrateur."
      ),
      myTechs.map(function(tech,i){
        var col=COLORS[i%COLORS.length], isSel=selTech&&selTech.id===tech.id;
        var initials=(tech.name||'?').split(' ').map(function(w){return w[0]||'';}).join('').slice(0,2).toUpperCase();
        return e('div',{key:tech.id,onClick:function(){setSelTech(isSel?null:tech);},
          style:{background:isSel?'#0f2040':'#070f20',borderRadius:10,padding:'12px 14px',marginBottom:8,border:'1px solid '+(isSel?col+'60':'#1e3a5f'),cursor:'pointer'}},
          e('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:isSel?10:0}},
            e('div',{style:{width:36,height:36,borderRadius:'50%',background:col,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:800,color:'#fff',flexShrink:0}},initials),
            e('div',{style:{flex:1}},
              e('div',{style:{fontSize:14,fontWeight:700,color:isSel?'#e2e8f0':'#94a3b8'}},tech.name),
              e('div',{style:{fontSize:11,color:'#475569'}},(tech.phone?'📞 '+tech.phone+' ':'')+(tech.defaultEmail?'✉ '+tech.defaultEmail:''))
            ),
            e('div',{style:{fontSize:10,color:col,background:col+'20',padding:'2px 8px',borderRadius:6}},isSel?'▼ Masquer':'▶ Rapports')
          ),
          isSel&&e('div',{style:{background:'#0a1628',borderRadius:8,padding:'10px 12px',fontSize:12,color:'#94a3b8',marginBottom:10}},
            e('div',{style:{display:'grid',gridTemplateColumns:'auto 1fr',gap:'4px 12px'}},
              ...[['Identifiant',tech.username||'—'],['Tél.',tech.phone||'—'],['Email',tech.defaultEmail||'—'],
                  ['Statut',tech.active?'✅ Actif':'❌ Désactivé'],['Mobile',tech.deviceId_mobile?'📱 Associé':'Non'],
                  ['PC',tech.deviceId_desktop?'🖥️ Associé':'Non']].map(function(row,j){
                return [e('div',{key:'l'+j,style:{fontWeight:600,color:'#64748b'}},row[0]+' :'),
                        e('div',{key:'v'+j,style:{color:'#e2e8f0'}},row[1])];
              }).flat()
            )
          ),
          isSel&&e('div',null,
            loading&&e('div',{style:{textAlign:'center',padding:12,color:'#334155'}},'⏳ Chargement...'),
            audits&&audits.length===0&&e('div',{style:{textAlign:'center',padding:12,color:'#334155',fontSize:12}},'Aucun rapport trouvé.'),
            audits&&audits.slice(0,20).map(function(a,ai){
              var g=a.garde||{};
              var meta={audit:{icon:'📡',color:'#3b82f6'},doe:{icon:'📋',color:'#22c55e'},
                antenne:{icon:'📶',color:'#f97316'},audit_cradlepoint:{icon:'🛰️',color:'#f59e0b'}}[a.type]||{icon:'📄',color:'#475569'};
              return e('div',{key:a.id,style:{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',background:'#0a1628',borderRadius:8,marginBottom:4,border:'1px solid #1e3a5f'}},
                e('span',{style:{fontSize:14}},meta.icon),
                e('div',{style:{flex:1,minWidth:0}},
                  e('div',{style:{fontSize:12,fontWeight:700,color:meta.color,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}},g.raisonSociale||g.ot||a.id),
                  e('div',{style:{fontSize:10,color:'#475569'}},(g.date||'')+(g.ot?' — OT '+g.ot:''))
                ),
                e('div',{style:{fontSize:10,fontWeight:700,color:a.status==='Terminé'?'#22c55e':'#f59e0b',flexShrink:0}},a.status||'En cours')
              );
            })
          )
        );
...
function AdminMiniModal(props) {
  var onClose=props.onClose, onSuccess=props.onSuccess;
  ...
  function handleReset() {
    if (!checkRecoveryCode(resetCode)) { setResetErr('Code incorrect'); return; }
    var newHash=hashPwd('5671');
    localStorage.setItem('__omar_pwd__',newHash);
    var profiles=getProfiles();
    saveProfiles(profiles.map(function(p){
      if(p.isAdmin) return Object.assign({},p,{pwdHash:newHash,needSetup:false});
      if(p.username==='5671') return Object.assign({},p,{deviceId:null,deviceInfo:null,deviceId_mobile:null,deviceInfo_mobile:null,deviceId_desktop:null,deviceInfo_desktop:null});
      return p;
    }));
    setShowReset(false); setErr(''); setPwd('');
    alert('✅ Mot de passe admin réinitialisé à "5671" et appareil du compte 5671 réinitialisé.');
  }

  function handleLogin() {
    var profiles=getProfiles();
    var adminProf=profiles.find(function(p){return p.isAdmin;})
      || profiles.find(function(p){return p.username==='5671'||p.id===ADMIN_ID||p.id==='admin_omar';});
    // Forcer isAdmin si trouvé par username/id
    if(adminProf && (!adminProf.isAdmin || adminProf.role!=='admin')){
      adminProf=Object.assign({},adminProf,{isAdmin:true,role:'admin'});
      saveProfiles(profiles.map(function(p){return p.id===adminProf.id?adminProf:p;}));
    }
    // Dernier recours: créer profil admin depuis __omar_pwd__
    if(!adminProf){
      var _fallbackPwd = localStorage.getItem('__omar_pwd__')||'yhsoxs';
      adminProf={id:ADMIN_ID,username:'5671',name:'Administrateur',
        isAdmin:true,role:'admin',active:true,pwdHash:_fallbackPwd,needSetup:false};
      var _profs2=profiles.concat([adminProf]);
      saveProfiles(_profs2);
    }
    if(!adminProf){setErr('Profil admin introuvable');return;}
    if(!pwd.trim()){setErr('Mot de passe requis');return;}
    if(hashPwd(pwd.trim())!==adminProf.pwdHash){setErr('Mot de passe incorrect');return;}
    onSuccess(adminProf);
  }

  var INP={width:'100%',padding:'9px 12px',borderRadius:8,border:'1px solid #334155',
    background:'#0a1628',color:'#e2e8f0',fontSize:13,boxSizing:'border-box',
    outline:'none',fontFamily:'inherit',marginBottom:6};

  return e('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',zIndex:10000,display:'flex',alignItems:'center',justifyContent:'center'},
    onClick:function(ev){if(ev.target===ev.currentTarget)onClose();}},
    e('div',{style:{background:'#0f2040',borderRadius:16,padding:24,width:290,border:'1px solid #7f1d1d',boxShadow:'0 8px 40px rgba(0,0,0,0.8)'}},
      e('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:20}},
        e('div',{style:{fontSize:24}},'🔐'),
        e('div',null,
          e('div',{style:{fontSize:15,fontWeight:700,color:'#e2e8f0'}},'Accès Administrateur'),
          e('div',{style:{fontSize:11,color:'#ef4444'}},'Zone sécurisée')
        )
      ),
      !showReset?e('div',null,
        e('input',{type:'password',value:pwd,onChange:function(ev){setPwd(ev.target.value);setErr('');},
          onKeyDown:function(ev){if(ev.key==='Enter')handleLogin();},
          placeholder:'Mot de passe admin',autoFocus:true,
          style:Object.assign({},INP,{border:'1px solid '+(err?'#ef4444':'#334155')})}),
        err&&e('div',{style:{fontSize:12,color:'#ef4444',marginBottom:8,textAlign:'center'}},err),
        e('div',{style:{display:'flex',gap:8,marginBottom:12}},
          e('button',{onClick:onClose,style:{flex:1,padding:'10px',borderRadius:10,border:'1px solid #334155',background:'transparent',color:'#64748b',cursor:'pointer',fontSize:13}},'Annuler'),
          e('button',{onClick:handleLogin,style:{flex:1,padding:'10px',borderRadius:10,border:'none',background:'#ef4444',color:'#fff',cursor:'pointer',fontWeight:700,fontSize:13}},'Connexion')
        ),
        e('div',{onClick:function(){setShowReset(true);setResetCode('');setResetErr('');},
          style:{textAlign:'center',fontSize:11,color:'#475569',cursor:'pointer',textDecoration:'underline'}},
          'Mot de passe oublié ?')
      ):e('div',null,
        e('div',{style:{fontSize:11,color:'#94a3b8',marginBottom:10,textAlign:'center'}},'Code de récupération (défaut: 1234)'),
        e('input',{type:'text',value:resetCode,onChange:function(ev){setResetCode(ev.target.value);setResetErr('');},
          onKeyDown:function(ev){if(ev.key==='Enter')handleReset();},
          placeholder:'Code de récupération',style:INP}),
        resetErr&&e('div',{style:{fontSize:11,color:'#ef4444',marginBottom:6,textAlign:'center'}},resetErr),
        e('div',{style:{display:'flex',gap:6}},
          e('button',{onClick:function(){setShowReset(false);},style:{flex:1,padding:'8px',borderRadius:8,border:'1px solid #334155',background:'transparent',color:'#64748b',cursor:'pointer',fontSize:12}},'Retour'),
          e('button',{onClick:handleReset,style:{flex:1,padding:'8px',borderRadius:8,border:'none',background:'#22c55e',color:'#fff',cursor:'pointer',fontWeight:700,fontSize:12}},'Réinitialiser')
        )
      )
    )
  );
...
function AuditListScreen({ onOpen, onCreate, profile, onSwitchProfile, onOpenAdmin }) {
  const [audits, setAudits] = useState(null);
  const [showProfile,    setShowProfile]    = useState(false);
  const [currentProfile, setCurrentProfile] = useState(profile);
  const [showResponsable,setShowResponsable]= useState(false);
  const [showAdminModal, setShowAdminModal] = useState(false);
  var isResponsable = profile && profile.isResponsable && !profile.isAdmin;
  ...
  if (showAdminModal) {
    return React.createElement(AdminMiniModal, {
      ...
      onSuccess: function(adminProf){
        ...
        if (typeof onOpenAdmin === 'function') onOpenAdmin(adminProf);
        else { window.__pendingAdminLogin=adminProf; onSwitchProfile(); }
      },
    });
  }
  if (showResponsable && isResponsable) {
    return React.createElement(ResponsableScreen, {
      profile: profile, onClose: function(){ setShowResponsable(false); },
    });
  }
  if (showProfile && currentProfile) {
    return React.createElement(ProfileScreen, {
      profile: currentProfile,
      onSave: function(updated){ setCurrentProfile(updated); },
      onClose: function(){ setShowProfile(false); },
    });
  }

    return (
    React.createElement('div', { style: {maxWidth:430,margin:"0 auto",background:"#020817",minHeight:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#f1f5f9"}}
      , React.createElement('div', { style: {background:"linear-gradient(135deg,#0f2040 0%,#1e3a5f 100%)",padding:"28px 20px 20px",borderBottom:"1px solid #1e3a5f"}}
        , React.createElement('div', { style: {display:"flex",alignItems:"center",gap:12}}
          ...
          , React.createElement('div', { style:{flex:1,minWidth:0}}
            , React.createElement('div', { style: {fontSize:20,fontWeight:800,color:"#f1f5f9"}}, "Audit PICO BTS"  )
          ...
          React.createElement('button',{
            ...
            style:{background:"#450a0a",border:"1px solid #ef4444",borderRadius:10,padding:"6px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,flexShrink:0}},
          ...
          isResponsable&&React.createElement('button',{
            ...
            style:{background:"#0c2040",border:"1px solid #1e4080",borderRadius:10,padding:"6px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,flexShrink:0}},
            React.createElement('div',{style:{fontSize:16}},'👥'),
            React.createElement('div',{style:{fontSize:8,color:"#60a5fa"}},'Équipe')
          ),
          React.createElement('button',{
            onClick:function(){ setShowProfile(true); },
            style:{background:"#1e3a5f",border:"1px solid #1e3a5f",borderRadius:10,padding:"6px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,flexShrink:0}},
            React.createElement('div',{style:{fontSize:16}},'⚙️'),
            React.createElement('div',{style:{fontSize:8,color:"#93c5fd"}},'Profil')
          ),
          React.createElement('button',{
            onClick:onSwitchProfile,
            style:{background:"#0f2040",border:"1px solid #1e3a5f",borderRadius:10,padding:"6px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,flexShrink:0}},
            React.createElement('div',{style:{width:32,height:32,borderRadius:"50%",background:(typeof getAvatarColor!=="undefined"?getAvatarColor(profile&&profile.name||'?'):"#3b82f6"),display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#fff"}},(profile&&profile.name||'?').charAt(0).toUpperCase()),
      ...
      , React.createElement('div', { style: {padding:"20px 18px"}}
        ...
        , React.createElement('div', { style: {fontSize:11,fontWeight:700,color:"#475569",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:12}}, "Mes audits "
            , audits!==null?`(${audits.length})`:""
        )
        , audits===null ? (
          React.createElement('div', { style: {textAlign:"center",padding:40,color:"#334155"}}, "Chargement...")
        ) : audits.length===0 ? (
          React.createElement('div', { style: {textAlign:"center",padding:40,color:"#334155"}}
            , React.createElement('div', { style: {fontSize:48,marginBottom:12}}, "📂")
            , React.createElement('div', { style: {fontSize:14}}, "Aucun audit enregistré"  )
            , React.createElement('div', { style: {fontSize:12,marginTop:6,color:"#1e3a5f"}}, "Créez votre premier audit ci-dessus"    )
          )
        ) : audits.slice().reverse().map(a=>(
          React.createElement('div', { key: a.id, onClick: ()=>onOpen(a.id, a.type||'audit'), style: {background:"#0f2040",borderRadius:14,padding:"16px",marginBottom:12,border:"1px solid #1e3a5f",cursor:"pointer",display:"flex",alignItems:"center",gap:14}}
            , React.createElement('div', { style: {background:"#1e3a5f",borderRadius:10,width:44,height:44,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}, "🏢")
            , React.createElement('div', { style: {flex:1,minWidth:0}}
              , React.createElement('div', { style: {fontWeight:700,fontSize:15,color:"#e2e8f0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}, a.raisonSociale||"Sans titre")
              , React.createElement('div', { style: {fontSize:12,color:"#475569",marginTop:3}}, "OT : "  , a.ot||"—", " · "  , a.date)
              , React.createElement('div', { style: {fontSize:11,color:"#334155",marginTop:2}}
                , a.pmCount>0?`${a.pmCount} PM`:"0 PM", " · "  , a.picoCount>0?`${a.picoCount} PICO`:"0 PICO"
              )
            )
            , React.createElement('div', { style: {display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8,flexShrink:0}}
              , React.createElement('div', { style: {background:statusColor(a.status)+"25",color:statusColor(a.status),borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:700}}, a.status||"Brouillon"),
              a.type==='doe' && React.createElement('span',{style:{marginLeft:6,background:'#22c55e20',color:'#22c55e',borderRadius:6,padding:'1px 7px',fontSize:9,fontWeight:700}},'DOE')
              , React.createElement('button', { onClick: e=>handleDelete(a.id,e), style: {background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:16,padding:0}}, "🗑️")
            )
          )
        )))
      )
...
function AuditEditor({ auditId, onBack, profile, onSwitchProfile, auditType, onCreateDOE }) {
  ...
  var isAntenne = auditType === 'antenne';
  var activeSections = isAntenne ? ANTENNE_SECTIONS : SECTIONS;
  ...
  useEffect(() => {
    loadAudit(auditId).then(saved => {
      if (saved) { setData(saved.data); setStatus(saved.status||"En cours"); }
        ...
        var d = isAntenne ? defaultAntenneData() : defaultData();
        if (profile) {
  ...
  const addPhoto = (sk, p) => setData(d=>({...d,[sk]:{...d[sk],photos:[...d[sk].photos,p]}}));
  const deletePhoto = (sk, i) => setData(d=>({...d,[sk]:{...d[sk],photos:d[sk].photos.filter((_,idx)=>idx!==i)}}));
  const replacePhoto = (sk, i, src) => setData(d=>({...d,[sk]:{...d[sk],photos:d[sk].photos.map((p,idx)=>idx===i?{src,annotated:null}:p)}}));
  ...
  const handleSave = async (st=status) => {
    setSaveLabel("...");
    const pm = data.plan.markers.filter(m=>m.type==="pm").length;
    const pico = data.plan.markers.filter(m=>m.type==="pico").length;
    ...
    await saveAudit(auditId, { data, status:st }); syncRapport(auditId, auditType, data, st);
  ...
  if (!data) return React.createElement('div', { style: {maxWidth:430,margin:"0 auto",background:"#020817",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",color:"#334155",fontFamily:"system-ui"}}, "Chargement...");
  ...
  const picoCount = data.plan.markers.filter(m=>m.type==="pico").length;
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case "preambule": {
        var _setPreamb = function(k,v) { setData(function(d){ return Object.assign({},d,{preambule:Object.assign({},d.preambule,{[k]:v})}); }); };
        var _pb = data.preambule || {};
        var _pbIntro   = _pb.intro   !== undefined ? _pb.intro   : "L'audit de couverture mobile permet de déterminer le nombre d'antennes et leurs emplacements afin de garantir une qualité du réseau 4G optimal au sein de l'entreprise.\n\nElle survient à la suite d'une sollicitation du client qui rencontre des difficultés de connexion au réseau mobile.";
        var _pbImport  = _pb.important !== undefined ? _pb.important : "Les antennes Picos émettent exclusivement du réseau mobile 4G. Les terminaux doivent être compatibles VoLTE, avec le service activé sur la ligne mobile ainsi que le service Data.";
        return React.createElement('div', {},
          React.createElement('h2', {style:T.title}, "📋 Préambule"),
          React.createElement(Card, {},
            React.createElement('div', {style:{fontSize:11,fontWeight:700,color:'#64748b',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:6}}, 'Texte d\'introduction'),
            React.createElement('textarea', {
              value: _pbIntro,
              onChange: function(ev){ _setPreamb('intro', ev.target.value); },
              rows: 5,
              style: {width:'100%',padding:'11px 14px',borderRadius:10,border:'1px solid #1e3a5f',background:'#0a1628',color:'#e2e8f0',fontSize:13,resize:'vertical',boxSizing:'border-box',outline:'none',fontFamily:'inherit',lineHeight:1.7}
            })
          ),
          React.createElement(Card, {title:'⚠️ Important'},
            React.createElement('textarea', {
              value: _pbImport,
              onChange: function(ev){ _setPreamb('important', ev.target.value); },
              rows: 3,
              style: {width:'100%',padding:'11px 14px',borderRadius:10,border:'1px solid #f59e0b40',background:'#2d1505',color:'#fbbf24',fontSize:13,resize:'vertical',boxSizing:'border-box',outline:'none',fontFamily:'inherit',lineHeight:1.7}
            })
          )
        );
      }
      ...
      case "outils": {
        ...
        var _HDRS = ['Band','eNB','LCID','RSRP','RSRQ','SNR>10','UPLINK','DOWNLINK','PING'];
        var _COLS = ['band','enb','lcid','rsrp','rsrq','snr','ul','dl','ping'];
        ...
        return React.createElement('div', {},
          ...
          React.createElement(Card, {},
            React.createElement(Field, {
              ...
              value: data.outils.logiciels !== undefined ? data.outils.logiciels : 'Network Cell Info Lite (Jauge) / Speedtest (Debit)',
              onChange: function(v){ setData(function(d){ return Object.assign({},d,{outils:Object.assign({},d.outils,{logiciels:v})}); }); },
              placeholder:'ex: Network Cell Info Lite / Speedtest...'
            }),
            React.createElement(Field, {label:'Notes complémentaires', value:data.outils.notes, onChange:function(v){set('outils','notes',v);}, placeholder:'Observations sur les outils utilisés...', multiline:true})
          ),
          isAntenne && React.createElement(Card, {title:'📊 Mesures radio sur site'},
            ...
            _mesures.map((m,i)=> {
              var isAnt = i<3;
              return React.createElement('div', {key:i, style:{marginBottom:12,background:isAnt?'#0a1800':'#000a18',borderRadius:12,padding:12,border:'1px solid '+(isAnt?'#22c55e30':'#3b82f630')}},
                ...
                React.createElement('div', {style:{display:'grid',gridTemplateColumns:'repeat(9,1fr)',gap:3,marginBottom:3}},
                  _HDRS.map(h=>React.createElement('div',{key:h,style:{fontSize:8,color:'#475569',textAlign:'center',fontWeight:700}},h))
                ),
                React.createElement('div', {style:{display:'grid',gridTemplateColumns:'repeat(9,1fr)',gap:3}},
      ...
      case "plan": return (
        React.createElement('div', {}
          , React.createElement('h2', { style: T.title}, "🗺️ Plan & Points de mesures"     )
          , React.createElement(Card, { title: "Plan interactif du site"   }
            , React.createElement(PlanEditor, {
              plan: data.plan,
              pmData: data.pmData,
              picoData: data.picoData,
              onChange: p=>setData(d=>({...d,plan:p})),
          ...
          , React.createElement(Card, { title: "Tableau de couverture radio"   }
            , React.createElement('div', { style: {background:"#0a1628",borderRadius:10,overflow:"hidden",marginBottom:14}}
              , [{label:"Bonne",value:"> -97dBm",color:"#22c55e"},{label:"Moyenne",value:"-98 < X < -107dBm",color:"#f59e0b"},{label:"Médiocre",value:"-108 < X < -117dBm",color:"#f97316"},{label:"Mauvaise",value:"> -118 dBm",color:"#ef4444"},{label:"Inexistante",value:"Pas de couverture",color:"#64748b"}].map(q=>(
                React.createElement('div', { key: q.label, style: {display:"flex",justifyContent:"space-between",padding:"8px 14px",borderBottom:"1px solid #1e3a5f"}}
          ...
          , React.createElement(Card, { title: `Données de mesure (${data.plan.markers.filter(m=>m.type==="pm").length} points)`}
            , React.createElement(PMTable, { plan: data.plan, pmData: data.pmData, onChange: pd=>setData(d=>({...d,pmData:pd}))} )
          )
        )
      );
      ...
      case "photos": return (
        React.createElement('div', {}
          , React.createElement('h2', { style: T.title}, "📸 Reporting Photos"  )
          , React.createElement(Card, {}
            , React.createElement('div', { style: {background:"#0a1628",borderRadius:10,padding:14,fontSize:13,color:"#64748b",lineHeight:1.7,marginBottom:16,borderLeft:"3px solid #3b82f6"}}, "Prendre des photos sur un plan suffisamment large pour permettre aux intervenants de retrouver aisément l'emplacement des picos."

            )
            , React.createElement(PhotoGallery, {
              photos: data.reportingPhotos.photos,
              onAdd: p=>addPhoto("reportingPhotos",p),
              onAnnotate: i=>setAnnotating({type:"gallery",key:"reportingPhotos",index:i}),
              onDelete: i=>deletePhoto("reportingPhotos",i),
              onReplace: i=>pickPhoto(src=>replacePhoto("reportingPhotos",i,src))}
            )
          )
        )
      );
      ...
      case "oeuvre": {
        ...
        const rawTotal = cableDetails.reduce(function(s,c) {
          var ov = (o.cableOverrides||{})[c.label]||{};
          var h = ov.meters !== undefined ? (parseFloat(ov.meters)||0) : c.meters;
          var v = ov.height !== undefined ? (parseFloat(ov.height)||0) : c.height;
          return s + h + v;
        }, 0);
        ...
        return (
          React.createElement('div', {}
            ...
            , React.createElement(Card, { title: "📐 Calcul du câblage Ethernet"    }
              , picoMarkers.length === 0 ? (
                ...
                React.createElement(React.Fragment, null
                  , cableDetails.map(function(c) {
                    var _overrides = o.cableOverrides || {};
                    var _ov = _overrides[c.label] || {};
                    var _dispH = _ov.meters !== undefined ? _ov.meters : c.meters;
                    var _dispV = _ov.height !== undefined ? _ov.height : c.height;
                    var _dispTotal = parseFloat(_dispH||0) + parseFloat(_dispV||0);
                    var INP_S = {padding:'4px 6px',borderRadius:6,border:'1px solid #1e3a5f',background:'#020817',color:'#93c5fd',fontSize:12,fontWeight:700,width:56,textAlign:'center',outline:'none',boxSizing:'border-box'};
                    var updOv = function(key, val) {
                      var newOv = Object.assign({}, _overrides, {[c.label]: Object.assign({}, _ov, {[key]: val === '' ? undefined : parseFloat(val)||0})});
                      upd('cableOverrides', newOv);
                    };
                    return React.createElement('div', { key: c.label, style: {background:"#0a1628",borderRadius:10,padding:"10px 14px",marginBottom:6,border:"1px solid #1e3a5f"}},
                      React.createElement('div', { style: {display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}},
                        ...
                        React.createElement('div', { style: {fontSize:15,fontWeight:800,color:_dispTotal>0?"#f1f5f9":"#475569"}}, _dispTotal>0 ? _dispTotal+' m' : '—')
                      ),
                      React.createElement('div', { style: {display:"flex",gap:10,alignItems:"center",flexWrap:'wrap'}},
                        React.createElement('div', { style: {display:"flex",alignItems:"center",gap:5,flex:1}},
                          React.createElement('span', {style:{fontSize:11,color:'#64748b',flexShrink:0}}, '🔌 Horizontal :'),
                          React.createElement('input', {type:'number', min:0, value:_dispH===0&&_ov.meters===undefined?'':_dispH,
                            onChange:function(ev){updOv('meters',ev.target.value);},
                            placeholder:c.meters||'0', style:INP_S}),
                          React.createElement('span', {style:{fontSize:11,color:'#475569'}}, 'm')
                        ),
                        React.createElement('div', { style: {display:"flex",alignItems:"center",gap:5,flex:1}},
                          React.createElement('span', {style:{fontSize:11,color:'#64748b',flexShrink:0}}, '📏 Hauteur :'),
                          React.createElement('input', {type:'number', min:0, value:_dispV===0&&_ov.height===undefined?'':_dispV,
                            onChange:function(ev){updOv('height',ev.target.value);},
                            placeholder:c.height||'0', style:INP_S}),
                          React.createElement('span', {style:{fontSize:11,color:'#475569'}}, 'm')
                        ),
                        (_ov.meters!==undefined||_ov.height!==undefined)&&React.createElement('button', {
                          onClick:function(){ upd('cableOverrides', Object.assign({},_overrides,{[c.label]:undefined})); },
                          style:{padding:'3px 8px',borderRadius:6,border:'none',background:'#450a0a',color:'#ef4444',fontSize:10,cursor:'pointer'}
                        }, '↩ Auto')
                      )
                    );
                  })
            ...
            /* ── ÉQUIPEMENTS / OUTILS ── */
            , React.createElement(Card, { title: "📏 Hauteur d\'installation" },
              React.createElement('div', {style:{marginBottom:6,padding:'10px 14px',background:'#0a1628',borderRadius:10,border:'1px solid #1e3a5f'}},
                maxHauteurAuto > 0 && React.createElement('div', {style:{fontSize:11,color:'#64748b',marginBottom:8}},
                  '📌 Auto depuis les PICOs : '+maxHauteurAuto+' m'),
                React.createElement('div', {style:{display:'flex',alignItems:'center',gap:8}},
                  React.createElement('input', {
                    type:'number', min:0, step:0.5,
                    value: o.hauteurInstallation !== undefined ? o.hauteurInstallation : (maxHauteurAuto||''),
                    onChange: function(ev){ upd('hauteurInstallation', ev.target.value==='' ? undefined : parseFloat(ev.target.value)||0); },
                    placeholder: maxHauteurAuto ? String(maxHauteurAuto) : 'ex: 3.5',
                    style:{flex:1,padding:'10px 12px',borderRadius:8,border:'1px solid #3b82f660',background:'#020817',color:'#93c5fd',fontSize:15,fontWeight:700,outline:'none',textAlign:'center',boxSizing:'border-box'}
                  }),
                  React.createElement('span', {style:{fontSize:13,color:'#64748b',flexShrink:0}}, 'm'),
                  o.hauteurInstallation !== undefined && React.createElement('button', {
                    onClick:function(){ upd('hauteurInstallation', undefined); },
                    style:{padding:'8px 10px',borderRadius:8,border:'none',background:'#450a0a',color:'#ef4444',cursor:'pointer',fontSize:11}
                  }, '↩ Auto')
                )
              )
            )
            ...
            , React.createElement(Card, { title: `📦 Fournitures${nb>0?` (base ${nb} PICO${nb>1?"s":""})`:"" }`}
              , fournituresAuto.map(f=>(
                React.createElement('div', { key: f.key, style: {display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"#0a1628",borderRadius:10,marginBottom:6,border:`1px solid ${(o.fournitures||{})[f.key]?"#3b82f660":"#1e3a5f"}`},
                  ...
                  , React.createElement('div', {style:{display:'flex',alignItems:'center',gap:4,flexShrink:0}, onClick:function(ev){ev.stopPropagation();}},
                    React.createElement('input', {
                      type:'number', min:0,
                      value: (o.fournituresQty && o.fournituresQty[f.key] !== undefined) ? o.fournituresQty[f.key] : f.qty,
                      onChange: function(ev){
                        ev.stopPropagation();
                        var newQty = Object.assign({}, o.fournituresQty||{}, {[f.key]: parseInt(ev.target.value)||0});
                        upd('fournituresQty', newQty);
                      },
                      style:{width:52,padding:'4px 6px',borderRadius:6,border:'1px solid #f59e0b60',background:'#1a0f00',color:'#f59e0b',fontSize:13,fontWeight:700,textAlign:'center',outline:'none'}
                    }),
                    React.createElement('span', {style:{fontSize:11,color:'#f59e0b',flexShrink:0}}, f.unit)
                  )
                )
              ))
  ...
  return (
    React.createElement('div', { style: {maxWidth:430,margin:"0 auto",background:"#020817",minHeight:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#f1f5f9",position:"relative"}}
      /* Import PDF modal */
          ...
          , React.createElement('div', { style: {width:280,background:"#0f2040",borderRight:"1px solid #1e3a5f",display:"flex",flexDirection:"column",overflowY:"auto"}}
            ...
            , profile && React.createElement('div', { style:{display:"flex",alignItems:"center",gap:8,marginTop:10,paddingTop:8,borderTop:"1px solid #1e3a5f"}},
              ...
              , React.createElement('div', { style: {fontSize:11,color:"#475569",marginTop:2}}
                , data.plan.markers.filter(m=>m.type==="pm").length, " PM · "   , data.plan.markers.filter(m=>m.type==="pico").length, " PICO"
              )
            )
            , (function(){var _isFin=isDOE&&data&&data.doe&&data.doe.finalise; return (isDOE ? DOE_SECTIONS : SECTIONS).map(sec=>{var _d=_isFin&&(sec.id==="oeuvre"||sec.id==="acces"); return React.createElement('button', { key: sec.id, onClick: _d?function(){}:()=>{setActiveSection(sec.id);setShowNav(false);},
                ...
                , sec.id==="plan" && data.plan.markers.filter(m=>m.type==="pm").length>0 && React.createElement('span', { style: {marginLeft:"auto",background:"#3b82f6",color:"#fff",borderRadius:10,padding:"1px 7px",fontSize:11,fontWeight:700}}, data.plan.markers.filter(m=>m.type==="pm").length)
              ); }); })()
            , React.createElement('div', { style: {marginTop:"auto",padding:16,display:"flex",flexDirection:"column",gap:8,borderTop:"1px solid #1e3a5f"}}
              ...
              , React.createElement('button', { onClick: ()=>{setShowPreview(true);setShowNav(false);}, style: {padding:"12px",borderRadius:10,border:"1px solid #1e3a5f",background:"transparent",color:"#93c5fd",cursor:"pointer",fontWeight:600,fontSize:14}}, "👁️ Aperçu PDF"  )
            , isDOE && React.createElement('button', { onClick: ()=>{setShowImportPDF(true);setShowNav(false);},
              ...
              , React.createElement('button', { id:"export-btn", onClick: async ()=>{ var btn=document.getElementById('export-btn'); if(btn){btn.textContent='⏳ PDF...';btn.disabled=true;} try{ await window.generatePDF({...data, docType: auditType}); if(btn){btn.textContent='✅ Téléchargé!';setTimeout(function(){btn.textContent='📄 Exporter PDF';btn.disabled=false;},3000);} }catch(e){ alert('Erreur PDF: '+e.message); if(btn){btn.textContent='📄 Exporter PDF';btn.disabled=false;} } setShowNav(false); }, style: {padding:"12px",borderRadius:10,border:"none",background:"#22c55e",color:"#fff",cursor:"pointer",fontWeight:700,fontSize:14,width:"100%"}}, "📄 Exporter PDF"  )
            , isDOE && React.createElement('button', { onClick: async ()=>{
                var b=event.currentTarget;b.textContent='⏳ PDF...';b.disabled=true;
                try{await window.generateDoePDF(data);b.textContent='✅ OK!';setTimeout(function(){b.textContent='📋 Exporter DOE PDF';b.disabled=false;},3000);}
                catch(e){alert('Erreur: '+e.message);b.textContent='📋 Exporter DOE PDF';b.disabled=false;}
                setShowNav(false);
      ...
      /* Bottom tab bar */
      , React.createElement('div', { style: {position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#0f2040",borderTop:"1px solid #1e3a5f",display:"flex",zIndex:100,overflowX:"auto",paddingBottom:"var(--safe-bottom, env(safe-area-inset-bottom, 0px))"}}
        , (isDOE ? DOE_SECTIONS : SECTIONS).map(sec=>(
          React.createElement('button', { key: sec.id, onClick: ()=>setActiveSection(sec.id),
            style: {flex:"0 0 auto",padding:"10px 10px 8px",border:"none",background:"transparent",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,borderBottom:activeSection===sec.id?"2px solid #3b82f6":"2px solid transparent",position:"relative"}}
            , React.createElement('span', { style: {fontSize:17}}, sec.icon)
            , React.createElement('span', { style: {fontSize:8,color:activeSection===sec.id?"#93c5fd":"#475569",fontWeight:600,whiteSpace:"nowrap"}}, sec.label.split(" ")[0])
            , sec.id==="pico" && picoCount>0 && React.createElement('span', { style: {position:"absolute",top:6,right:4,background:"#f97316",color:"#fff",borderRadius:8,padding:"0 4px",fontSize:9,fontWeight:800,lineHeight:"14px"}}, picoCount)
          )
        ))
      )
    )
...
function findProfileByUsername(username) {
  ...
  return profiles.find(function(p) {
    ...
    if (p.id && p.id.toLowerCase() === u) return true; // match par ID aussi
    var def = (function() {
...
function UsernameLoginScreen({ onSelect }) {
  // Pré-remplir depuis __pending_login__ si disponible
  var _pendingLogin = null;
  try { _pendingLogin = JSON.parse(localStorage.getItem('__pending_login__')||'null'); } catch(e){}
  var _initU = (_pendingLogin&&(_pendingLogin.username||_pendingLogin.id)) || '';
  var _sui = React.useState(_initU); var usernameInput = _sui[0]; var setUsernameInput = _sui[1];
  ...
  var handleUsernameLogin = function() {
    // Lire la valeur DOM si l'état React est vide (cas du pré-remplissage)
    var _inputU = usernameInput.trim() ||
      ((document.querySelector('input[autocomplete="username"]') ||
        document.querySelector('input[type="text"]') ||
        document.querySelector('input:not([type="password"])') || {}).value || '').trim();
    var _inputP = pwdInput ||
      ((document.querySelector('input[type="password"]') || {}).value || '');
    if (_inputU) { setUsernameInput(_inputU); }
    if (_inputP) { setPwdInput(_inputP); }
    var finalU = _inputU || usernameInput;
    var finalP = _inputP || pwdInput;

    var prof = findProfileByUsername(finalU);
    if (!prof) { setLoginError('Identifiant inconnu: "'+finalU+'"'); return; }
    // Forcer isAdmin si c'est un compte admin connu
    if (!prof.isAdmin && (prof.username==='5671' || prof.id===ADMIN_ID)) {
      prof = Object.assign({}, prof, {isAdmin:true});
      saveProfiles(getProfiles().map(function(p){ return p.id===prof.id ? prof : p; }));
    }
    var now = Date.now();
    if (prof.expiryDate) {
      var exp = new Date(prof.expiryDate).getTime();
      if (!isNaN(exp) && now > exp) { setLoginError('Profil expiré'); return; }
    }
    var hash = finalP ? hashPwd(finalP) : null;
...
function ProfileSelectorScreen({ onSelect, initialAdminSession, onClose }) {
  ...
  var _s8 = useState({ name:'', phone:'', username:'', pwd:'', pwd2:'', expiry:'', role:'technicien' });
  ...
  var handleLogin = function() {
    ...
    // Forcer isAdmin pour l'identifiant admin connu
    if (!loginProf.isAdmin && (loginProf.username==='5671' || loginProf.id==='admin_omar' || loginProf.id===ADMIN_ID)) {
      loginProf = Object.assign({}, loginProf, {isAdmin:true});
      var allP = getProfiles();
      saveProfiles(allP.map(function(p){ return p.id===loginProf.id ? loginProf : p; }));
    }
  ...
  var handleAddProfile = function() {
    ...
    var uname = newProf.username.trim() || todayUsername;
    // Vérifier que le login n'existe pas déjà
    if (profiles.find(function(p){ return p.username===uname; })) { alert('Cet identifiant est déjà utilisé.'); return; }
    if (newProf.pwd.length < 4) { alert('Code secret minimum 4 chiffres.'); return; }
    if (newProf.pwd !== newProf.pwd2) { alert('Les codes secrets ne correspondent pas.'); return; }
    var exists = profiles.find(function(p){ return p.name.toLowerCase()===name.toLowerCase(); });
    if (exists) { alert('Ce nom existe déjà.'); return; }
    var uname = newProf.username.trim() || todayUsername;
    if (profiles.find(function(p){ return p.username===uname; })) { alert('Cet identifiant existe déjà.'); return; }
    ...
    var newRec = {
      ...
      username:uname,
      isAdmin: (newProf.role||'technicien')==='admin',
      isResponsable:(newProf.role||'technicien')==='responsable',
      active:  true,
      pwdHash: hashPwd(newProf.pwd),
      needSetup: false,
      expiryDate: newProf.expiry.trim() || null,
      createdAt: new Date().toISOString(),
    };
    var updated = profiles.concat([newRec]);
    saveProfiles(updated); setProfiles(updated);
    setShowAdd(false); setNewProf({name:'',phone:'',username:'',pwd:'',pwd2:'',expiry:'',role:'technicien'});

    // Sync vers api.php (silencieux si hors ligne)
    var apiKey = (typeof API_KEY!=='undefined')?API_KEY:'';
    var apiBase= (typeof API_BASE!=='undefined')?API_BASE:'./api.php';
    if (apiKey) {
      fetch(apiBase+'?action=add_profile&k='+encodeURIComponent(apiKey), {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          name:     newRec.name,
          username: newRec.username,
          phone:    newRec.phone,
          pwd_hash: newRec.pwdHash,
          role:     (newProf.role||'technicien'),
        })
      }).catch(function(){});
    }
  };
  ...
  var handleSaveEdit = function(resetDevice) {
    ...
    var updated = profiles.map(function(p){
      ...
      var next = Object.assign({}, p, {
        name: name, phone: editForm.phone.trim(),
        ...
        defaultEmail:  editForm.defaultEmail||null,
      });
      if(editForm.pwd) next.pwdHash = hashPwd(editForm.pwd);
      if(resetDevice) { next.deviceId=null; next.deviceInfo=null; next.deviceId_mobile=null; next.deviceId_desktop=null; }
  ...
  // ── Render ───────────────────────────────────────────────────────────────
  return React.createElement('div', { style:{maxWidth:430,margin:'0 auto',minHeight:'100vh',
    ...
    showAdmin && adminSession && React.createElement('div', { style:{position:'fixed',inset:0,
      ...
      fontFamily:'inherit',color:'#f1f5f9',maxWidth:430,margin:'0 auto'} },
      ...
      React.createElement('div', { style:{flex:1,overflowY:'auto',padding:'20px'} },
        ...
        showAdd && React.createElement('div', { style:{background:'#1a1040',borderRadius:12,
          ...
          React.createElement('input', { autoFocus:true, value:newProf.name,
            onChange:function(e){setNewProf(Object.assign({},newProf,{name:e.target.value}));},
          ...
          React.createElement('input', { type:'tel', value:newProf.phone,
            onChange:function(e){setNewProf(Object.assign({},newProf,{phone:e.target.value}));},
          ...
          React.createElement('input', { type:'date', value:newProf.expiry,
            onChange:function(e){setNewProf(Object.assign({},newProf,{expiry:e.target.value}));},
          ...
          React.createElement('label', { style:LBL }, 'Code secret (4 chiffres minimum)'),
          React.createElement('input', { type:'password', inputMode:'numeric', pattern:'[0-9]*', maxLength:4, value:newProf.pwd,
            onChange:function(e){setNewProf(Object.assign({},newProf,{pwd:e.target.value}));},
            placeholder:'••••', style:INP }),
          React.createElement('label', { style:LBL }, 'Confirmer le code secret'),
          React.createElement('input', { type:'password', inputMode:'numeric', pattern:'[0-9]*', maxLength:4, value:newProf.pwd2,
            onChange:function(e){setNewProf(Object.assign({},newProf,{pwd2:e.target.value}));},
            onKeyDown:function(e){if(e.key==='Enter')handleAddProfile();},
            placeholder:'Répéter le code', style:INP }),
          React.createElement('label', { style:LBL }, 'Rôle'),
          React.createElement('select', {
            value: newProf.role||'technicien',
            onChange: function(e){ setNewProf(Object.assign({},newProf,{role:e.target.value})); },
            style:INP
          },
            React.createElement('option',{value:'technicien'},'👷 Technicien'),
            React.createElement('option',{value:'responsable'},'👥 Responsable'),
          ...
          React.createElement('div', { style:{display:'flex',gap:8} },
            ...
            React.createElement('button', { onClick:function(){setShowAdd(false);setNewProf({name:'',phone:'',username:'',pwd:'',pwd2:'',expiry:'',role:'technicien'});}, style:BTN_N }, 'Annuler')
          )
        ),
        ...
        profiles.filter(function(p){return !p.isAdmin;}).map(function(prof){
          ...
          return React.createElement('div', { key:prof.id,
            ...
            React.createElement('div', { style:{display:'flex',alignItems:'center',gap:10,padding:'12px 14px'} },
              ...
              /* Edit */
              React.createElement('button', {
                onClick:function(){ setEditTarget(prof); setEditForm({name:prof.name,phone:prof.phone||'',username:prof.username||'',pwd:'',pwd2:'',expiry:prof.expiryDate||'',role:prof.isResponsable?'responsable':'technicien',responsableId:prof.responsableId||'',defaultEmail:prof.defaultEmail||''}); },
            ...
            isEdit && React.createElement('div', { style:{background:'#0a1628',padding:'14px',
              ...
              React.createElement('div',{style:{marginBottom:10}},
                ...
                React.createElement('div',{style:{display:'flex',gap:8}},
                  ['technicien','responsable'].map(function(role){
                    ...
                    return React.createElement('button',{key:role,onClick:function(){setEditForm(Object.assign({},editForm,{role:role}));},
                      style:{flex:1,padding:'8px',borderRadius:8,border:'none',cursor:'pointer',fontWeight:700,fontSize:12,
                        background:isSel?(role==='responsable'?'#7c3aed':'#0369a1'):'#1e3a5f',color:isSel?'#fff':'#64748b'}
              ...
              (editForm.role||'technicien')==='technicien'&&React.createElement('div',{style:{marginBottom:10}},
                ...
                React.createElement('select',{value:editForm.responsableId||'',onChange:function(ev){setEditForm(Object.assign({},editForm,{responsableId:ev.target.value}));},style:INP},
...
function App() {
  // Connexion synchrone si __pending_login__ est défini (depuis admin.php)
  var _syncLogin = null;
  try {
    var _pl = JSON.parse(localStorage.getItem('__pending_login__')||'null');
    if (_pl && _pl.id) {
      _pl.isAdmin = _pl.isAdmin || _pl.id===ADMIN_ID || _pl.username==='5671';
      _syncLogin = _pl;
      localStorage.removeItem('__pending_login__');
      localStorage.setItem('__active_profile_id__', _pl.id);
      window._activeProfile = _pl.id;
    }
  } catch(e) {}
  // Aussi vérifier __active_profile_id__ comme fallback
  if (!_syncLogin) {
    try {
      var _aid = localStorage.getItem('__active_profile_id__');
      if (_aid) {
        var _allP = JSON.parse(localStorage.getItem('__profiles__')||'[]');
        var _fp = _allP.find(function(p){ return p.id===_aid||p.username===_aid; });
        if (_fp && _fp.active!==false) {
          _fp.isAdmin = _fp.isAdmin || _fp.id===ADMIN_ID || _fp.username==='5671';
          _syncLogin = _fp;
          localStorage.removeItem('__active_profile_id__');
          window._activeProfile = _fp.id;
        }
      }
    } catch(e2) {}
  }

  var _useState = useState(_syncLogin), profile = _useState[0], setProfile = _useState[1];
  ...
  var _usAdmin = useState(null); var adminSession = _usAdmin[0]; var setAdminSession = _usAdmin[1];
  var handleOpenAdmin = function(adminProf) { setAdminSession(adminProf); };
  if (adminSession) {
    return React.createElement(ProfileSelectorScreen, {
      onSelect: function(){ setAdminSession(null); },
  ...
  // Auto-login depuis reset_db.php ou admin.php
  React.useEffect(function(){
    if (profile) return;

    // Vérifier ?login= param (depuis admin.php)
    try {
      var params = new URLSearchParams(window.location.search);
      var lp = params.get('login');
      if (lp) {
        window.history.replaceState({}, '', window.location.pathname);
        var allP = getProfiles();
        var found = allP.find(function(p){ return p.id===lp||p.username===lp; });
        if (!found) {
          // Créer profil admin si c'est admin_omar
          if (lp === 'admin_omar' || lp === '5671') {
            found = { id: ADMIN_ID, username: '5671', name: 'Administrateur',
              isAdmin: true, active: true, role: 'admin',
              pwdHash: localStorage.getItem('__omar_pwd__')||'yhsoxs', needSetup: false };
            allP.push(found);
            saveProfiles(allP);
          }
        }
        if (found) {
          if (found.id===ADMIN_ID||found.username==='5671') found=Object.assign({},found,{isAdmin:true,role:'admin'});
    ...
    // Vérifier __pending_login__ (depuis admin.php ou reset_db.php)
    var pending = null;
    try { pending = JSON.parse(localStorage.getItem('__pending_login__')||'null'); } catch(e){}
    if (pending && pending.id) {
      localStorage.removeItem('__pending_login__');
      // S'assurer que le profil est dans la liste locale
      var existingProfs = getProfiles();
      if (!existingProfs.find(function(p){ return p.id===pending.id; })) {
        existingProfs.push(pending);
        saveProfiles(existingProfs);
      }
      // Forcer isAdmin si c'est un admin connu
      if (!pending.isAdmin && (pending.username==='5671'||pending.id===ADMIN_ID)) {
        pending = Object.assign({}, pending, {isAdmin:true});
      }
      handleSelectProfile(pending);
      return;
    }

    // Vérifier __active_profile_id__
    var activeId = localStorage.getItem('__active_profile_id__');
    if (activeId) {
      var prof = getProfiles().find(function(p){ return p.id===activeId||p.username===activeId; });
  ...
  var createAudit = function(type) { setCurrentAuditId('audit_' + Date.now()); setCurrentType(type||'audit'); setScreen("editor"); };
  var createDoeFromAudit = async function(sourceId, sourceData) {
    ...
    var pmC   = (doeData.plan.markers||[]).filter(function(m){ return m.type==='pm'; }).length;
    var picoC = (doeData.plan.markers||[]).filter(function(m){ return m.type==='pico'; }).length;
  ...
  return React.createElement(AuditListScreen, { onOpen: openAudit, onCreate: createAudit, profile: profile, onSwitchProfile: handleSwitchProfile, onOpenAdmin: handleOpenAdmin });
}
...
async function syncFromServer() {
  try {
    ...
    // Force isAdmin sur admin_omar après sync serveur
    try {
      var _sp = JSON.parse(localStorage.getItem('__profiles__')||'[]');
      var _sc = false;
      _sp = _sp.map(function(p){
        if(p.id===ADMIN_ID||p.username==='5671'){
          _sc=true;
          return Object.assign({},p,{isAdmin:true,role:'admin'});
        }
        return p;
      });
      if(_sc) localStorage.setItem('__profiles__',JSON.stringify(_sp));
    } catch(e){}
  } catch(e) {}
}
...
function mountApp() {
  ...
  // Pre-check: si active_username='5671' ou admin_omar → forcer isAdmin dans __profiles__
  try {
    var _au = localStorage.getItem('active_username') || localStorage.getItem('__active_profile_id__');
    var _profs2 = JSON.parse(localStorage.getItem('__profiles__')||'[]');
    // Forcer isAdmin sur le profil admin dans __profiles__
    var _changed = false;
    _profs2 = _profs2.map(function(p){
      if(p.id===ADMIN_ID || p.username==='5671'){
        if(!p.isAdmin){ _changed=true; return Object.assign({},p,{isAdmin:true,role:'admin'}); }
      }
      return p;
    });
    if(_changed) localStorage.setItem('__profiles__', JSON.stringify(_profs2));
    // Si admin_omar pas dans la liste, l'ajouter
    if(!_profs2.find(function(p){return p.id===ADMIN_ID;})) {
      var _omarPwd = localStorage.getItem('__omar_pwd__')||'yhsoxs';
      _profs2.unshift({id:ADMIN_ID,username:'5671',name:'Administrateur',
        phone:'',isAdmin:true,active:true,pwdHash:_omarPwd,needSetup:false,role:'admin'});
      localStorage.setItem('__profiles__', JSON.stringify(_profs2));
    }
    if (_au) {
      var _fp = _profs2.find(function(p){ return p.id===_au||p.username===_au; });
      if (_fp) {
        _fp.isAdmin = _fp.isAdmin || _fp.id===ADMIN_ID || _fp.username==='5671';
        localStorage.setItem('__pending_login__', JSON.stringify(_fp));
      }
    }
  } catch(e2) {}
  // Force isAdmin sur le profil admin juste avant le render
  try {
    var profs = JSON.parse(localStorage.getItem('__profiles__') || '[]');
    var changed = false;
    profs = profs.map(function(p) {
      if (p.username === '5671' || p.id === 'admin_omar' || p.id === ADMIN_ID) {
    ...
    if (!profs.find(function(p){ return p.id === ADMIN_ID; })) {
      var _pwd = localStorage.getItem('__omar_pwd__') || 'yhsoxs';
      profs.unshift({ id: ADMIN_ID, username: '5671', name: 'Administrateur',
        phone: '', isAdmin: true, active: true, pwdHash: _pwd,
        needSetup: false, role: 'admin' });
      changed = true;
    }
    if (changed) localStorage.setItem('__profiles__', JSON.stringify(profs));
  } catch(e) {}
...
if (!window.__ADMIN_SKIP_MOUNT__) {
